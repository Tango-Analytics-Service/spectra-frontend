// src/components/channel-sets/CreateSmartSetDialog.tsx
...
import { SmartSetBuildCriteria } from "@/types/channel-sets";
import {
  createCardStyle,
  createButtonStyle,
  createTextStyle,
...
const CreateSmartSetDialog: React.FC<CreateSmartSetDialogProps> = ({
  ...
  const { createChannelSet } = useChannelSets();
  const { userFilters, fetchUserFilters } = useFilters();
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const newSet = await createChannelSet({
        name: name.trim(),
