...
import { ChannelDetails, ChannelSet, CreateChannelSetRequest, UpdateChannelSetRequest } from "@/types/channel-sets";
import { create } from "zustand";

export interface ChannelsSetsStore {
    channelsSets: ChannelSet[];
    isLoaded: boolean;
    totalSets: number;
    totalChannels: number;
    lastFetched: number;
    channelsSetsCache: Record<string, { data: ChannelSet; timestamp: number }>,
    // Methods for managing channel sets
    fetchChannelsSets: (forceRefresh?: boolean) => Promise<void>;
    getChannelsSet: (id: string) => Promise<ChannelSet | undefined>;
    createChannelsSet: (data: CreateChannelSetRequest) => Promise<ChannelSet | undefined>;
    updateChannelsSet: (id: string, data: UpdateChannelSetRequest) => Promise<ChannelSet | undefined>;
    ...
    refreshChannelsSet: (id: string) => Promise<ChannelSet | undefined>;
...
export const useChannelsSetsStore = create<ChannelsSetsStore>((set, getState) => ({
    ...
    getChannelsSet: async (id: string): Promise<ChannelSet | undefined> => {
        const state = getState();
    ...
    refreshChannelsSet: async (id: string): Promise<ChannelSet | undefined> => {
        try {
            const channelsSet = await channelSetService.getChannelSet(id);
            set(state => ({
                ...state,
    ...
    createChannelsSet: async (data: CreateChannelSetRequest): Promise<ChannelSet | undefined> => {
        try {
