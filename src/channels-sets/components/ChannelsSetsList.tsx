...
import { Users, Calendar, CheckCircle, AlertCircle, ArrowRight, Loader2, Globe, Lock, Star, Zap } from "lucide-react";
import { cn } from "@/lib/cn";
import { components, gradients, typography, spacing, animations, createCardStyle, createBadgeStyle } from "@/lib/design-system";
import { ChannelsSet } from "@/channels-sets/types";

export interface ChannelSetsListProps {
    channelSets: ChannelsSet[],
    isLoading: boolean,
        ...
        return (
            ...
            >
                {channelSets.map((set, index) => {
                    const isSmartSet = !!set.build_criteria;
                    ...
                    return (
                        <motion.div
                            ...
                            className={cn(
                                ...
                                isSmartSet && "border-yellow-500/30 bg-gradient-to-br from-yellow-500/5 to-transparent",
                            )}
                                ...
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        <div className="flex items-center">
                                            <h3 className={cn(typography.h4, "text-white mr-2 flex items-center gap-2")}>
                                                {isSmartSet && <Zap size={14} className="text-yellow-400" />}
                                                {set.name}
                                            </h3>
                                            {set.is_predefined && (
                                    ...
                                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-500/20 text-blue-400">
                                        {isSmartSet ? <Zap size={16} /> : <Users size={16} />}
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-2 text-xs mt-3">
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        {isSmartSet && set.build_status ? (
                                            ...
                                            // Обычный статус для обычных наборов
                                            <>
                                                {set.all_parsed ? (
                                                    <span
                                                        className={cn(
                    ...
                    >
                        <div
                            className={cn(
                                ...
                                isSmartSet ? "bg-gradient-to-br from-yellow-500 to-yellow-600" : gradients.primary,
                            )}
                        >
                            {isSmartSet ? <Zap size={16} /> : set.name.charAt(0).toUpperCase()}
                        </div>
                            ...
                            <div className="flex items-center">
                                <div
                                    className={cn(
                                        ...
                                        "text-white truncate max-w-[80%] flex items-center gap-2",
                                    )}
                                >
                                    {set.name}
                                    {isSmartSet && (
